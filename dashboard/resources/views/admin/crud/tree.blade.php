...
@php
    ...
    $renderNode = function ($item, $depth = 0) use (&$renderNode, $routePrefix, $maxDepth) {
        ...
        if ($children->isNotEmpty() && $maxDepth > 1) {
...
@section('content')
    <div class="page-header">
        <div>
            ...
            <p class="page-description">Kéo thả để sắp xếp vị trí và thứ bậc.</p>
        </div>
        <div class="actions">
    ...
    <div class="card p-4" style="padding: 20px;">
        ...
        @else
            <div class="dd" id="nestable-tree">
                <ol class="dd-list">
                    @php
                        // For flat lists, parent_id is null. For trees, root items have parent_id null.
                        $rootItems = $items->filter(function($i) { return empty($i->parent_id); })->sortBy('order_column');
                    @endphp
