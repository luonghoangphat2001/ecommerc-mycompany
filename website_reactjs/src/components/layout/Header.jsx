    ...
    <header className="h-20 bg-white/60 backdrop-blur-xl border-b border-white/40 shadow-[0_4px_30px_rgba(0,0,0,0.05)] sticky top-0 z-50 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto h-full flex items-center justify-between">
        ...
        <div className="flex-1 max-w-md px-8 hidden lg:block">
          <SearchBar placeholder={t('header.search_placeholder') || 'Tìm kiếm...'} />
        </div>
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          {!_hasHydrated ? (
            ...
            <Link 
              to="/login" 
              className="hidden sm:block ml-4 px-5 py-2 text-sm font-medium text-white bg-slate-900 rounded-full hover:bg-blue-600 transition-all shadow-md hover:shadow-lg transform hover:-translate-y-0.5"
            >
              {t('header.login')}
            </Link>
          )}
      ...
      {isMobileMenuOpen && (
        <div className="md:hidden absolute top-full left-0 w-full bg-white/95 backdrop-blur-2xl border-b border-slate-200 shadow-2xl animate-in slide-in-from-top duration-300">
          <div className="px-4 py-6 space-y-4">
            <NavLink to="/" onClick={closeMobileMenu}>{t('header.home')}</NavLink>
            <NavLink to="/shop" onClick={closeMobileMenu}>{t('header.shop')}</NavLink>
