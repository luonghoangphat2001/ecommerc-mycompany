...
import { Search, Truck, CheckCircle2, Clock, Box } from 'lucide-react';
          ...
          <form onSubmit={handleTrack} className="flex flex-col md:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <input 
                ...
                placeholder="Nhập mã đơn hàng (Ví dụ: NS-9923)"
            ...
            <button className="bg-blue-600 hover:bg-blue-700 text-white font-bold py-4 px-10 rounded-2xl transition-all shadow-lg shadow-blue-100 active:scale-95">
              Theo dõi ngay
            </button>
          </form>
        </Card.Body>
                ...
                {trackingInfo.steps.map((step, index) => (
                  <div key={index} className="flex items-start gap-6">
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center z-10 ${
                      step.completed ? 'bg-blue-600 text-white shadow-lg shadow-blue-100' : 'bg-white border-2 border-slate-100 text-slate-300'
                    }`}>
