...
import { User, Package, LogOut, ChevronRight, Lock, Truck } from 'lucide-react';
...
const AccountSidebar = ({ user, onLogout, activeTab, setActiveTab }) => {
  ...
  const menuItems = [
    { id: 'orders', label: t('account.orders_title'), icon: Package },
    { id: 'profile', label: t('account.profile_title'), icon: User },
    { id: 'password', label: t('account.password_title'), icon: Lock },
    { id: 'tracking', label: t('account.tracking_title'), icon: Truck },
  ];
    ...
    <aside className="w-full md:w-80 space-y-4">
      <div className="bg-white/60 backdrop-blur-xl border border-white/60 rounded-[2.5rem] p-8 shadow-[0_8px_30px_rgba(0,0,0,0.02)]">
        ...
        <nav className="space-y-2">
          ...
          <button 
            onClick={onLogout}
            className="w-full flex items-center justify-between p-4 text-red-500 hover:bg-red-50 rounded-2xl font-semibold transition-all mt-4"
          >
            <div className="flex items-center gap-3">
              <LogOut size={20} />
