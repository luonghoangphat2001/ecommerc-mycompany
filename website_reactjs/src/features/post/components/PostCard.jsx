...
const PostCard = ({ post, variant = 'default' }) => {
  ...
  if (isCompact) {
    return (
      <Link 
        to={`/posts/${slug}`} 
        className="flex gap-4 p-4 bg-white rounded-xl hover:bg-slate-50 transition-colors group"
      >
        {postImage ? (
          <img 
            src={postImage} 
        ...
        <div className="flex-1 min-w-0">
          {postCategory && (
            <span className="text-xs text-blue-600 font-medium">{postCategory.name}</span>
          )}
    ...
    return (
      <div className="flex flex-col md:flex-row bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-lg transition-all duration-300 group">
        ...
        <div className="md:w-3/5 p-6 md:p-8 flex flex-col">
          ...
          {postCategory && (
            <Link 
              to={`/posts?category=${postCategory.slug}`}
          ...
          <p className="text-slate-600 mb-4 line-clamp-3">
            {truncateText(excerpt || content, 200)}
          </p>
          ...
          {tags.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-4">
              ...
              {tags.length > 3 && (
                <span className="px-2 py-1 text-slate-400 text-xs">+{tags.length - 3}</span>
              )}
            </div>
          )}
    ...
    <article className="bg-white rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 group">
      ...
      <div className="relative aspect-video overflow-hidden">
        ...
        {postCategory && (
          <Link 
            to={`/posts?category=${postCategory.slug}`}
            className="absolute top-4 left-4 px-3 py-1 bg-white/90 backdrop-blur-sm text-blue-600 text-xs font-semibold rounded-full hover:bg-white transition-colors"
          >
            {postCategory.name}
          </Link>
        )}
      ...
      <div className="p-6">
        ...
        <p className="text-slate-600 text-sm mb-4 line-clamp-2">
          {truncateText(excerpt || content, 120)}
        </p>
