    ...
    <div className="min-h-screen bg-slate-50 flex items-center justify-center relative overflow-hidden font-sans">
      <div className="bg-white/60 backdrop-blur-xl p-8 md:p-12 w-full max-w-[450px] border border-white/60 shadow-[0_20px_50px_rgba(0,0,0,0.05)] rounded-[2.5rem] relative z-10 mx-4">
        <Link to="/login" className="inline-flex items-center gap-2 text-sm font-semibold text-slate-500 hover:text-blue-600 transition-colors mb-8">
          <ArrowLeft size={16} />
          {t('common.back_to_login') || 'Quay lại đăng nhập'}
        </Link>
        ...
        <div className="text-center mb-10">
          <h1 className="text-3xl font-black bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-purple-600 mb-3 tracking-tight">
            {t('auth.forgot_password_title') || 'Quên mật khẩu?'}
          </h1>
          <p className="text-slate-500 font-medium">{t('auth.forgot_password_subtitle') || 'Nhập email của bạn để nhận hướng dẫn đặt lại mật khẩu'}</p>
        </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            <label className="block text-sm font-semibold text-slate-700 mb-2 ml-1">{t('auth.email')}</label>
          ...
          >
            {isLoading ? t('auth.processing') : t('auth.send_request') || 'Gửi yêu cầu'}
          </button>
        </form>
      </div>
    </div>
