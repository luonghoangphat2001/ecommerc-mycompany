...
import { List, Grid3X3, Newspaper } from "lucide-react"
import PostCard from "./PostCard"
import Skeleton from "../../../components/common/Skeleton"

const PostList = ({ posts, loading, layout = "grid", columns = 3, showFeatured = true, onLayoutChange }) => {
    ...
    if (!posts || posts.length === 0) {
        return (
            <div className="text-center py-16">
                ...
                <p className="text-slate-500 text-lg">{translate?.("no_posts") || "Chưa có bài viết nào"}</p>
            </div>
        )
    }
                ...
                {onLayoutChange && (
                    <div className="flex items-center justify-between mb-6">
                        ...
                        <div className="flex items-center gap-2">
                            <button onClick={() => onLayoutChange("grid")} className={`p-2 rounded-lg transition-colors ${layout === "grid" ? "bg-blue-100 text-blue-600" : "hover:bg-slate-100"}`}>
                                <Grid3X3 size={20} />
                            </button>
                            <button onClick={() => onLayoutChange("list")} className={`p-2 rounded-lg transition-colors ${layout === "list" ? "bg-blue-100 text-blue-600" : "hover:bg-slate-100"}`}>
                                <List size={20} />
                            </button>
                        </div>
                    </div>
