...
const addressService = {
  ...
  getCountries: (config = {}) => {
    return apiService.get('addresses/countries', config);
  },

  getStates: (countryCode, config = {}) => {
    const url = `addresses/countries/${countryCode}/states`;
    return apiService.get(url, config);
  },

  getRegions: (countryCode, stateId, config = {}) => {
    const url = `addresses/countries/${countryCode}/states/${stateId}/regions`;
    return apiService.get(url, config);
  },

  getSubRegions: (countryCode, stateId, regionId, config = {}) => {
    const url = `addresses/countries/${countryCode}/states/${stateId}/regions/${regionId}/sub-regions`;
    return apiService.get(url, config);
  },
