...
const OrderDetail = ({ order, onBack }) => {
    ...
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-right duration-300">
            <button onClick={onBack} className="flex items-center gap-2 text-slate-500 hover:text-blue-600 transition-colors font-bold mb-4">
                <ArrowLeft size={20} />
                Quay lại danh sách
            </button>
                    ...
                    <Card className="shadow-sm border-slate-100">
                        ...
                        <Card.Body className="p-6 space-y-4">
                            ...
                            <div>
                                ...
                                <span className={`inline-block px-3 py-1 rounded-full text-xs font-bold ${order.status === "Đang xử lý" ? "bg-orange-100 text-orange-600" : "bg-green-100 text-green-600"}`}>{order.status}</span>
                            </div>
                        </Card.Body>
                    </Card>
